...
import cpabe.policyparser.ASTAreaAttribute;
import cpabe.policyparser.ASTAttribute;
import cpabe.policyparser.ASTExpression;
import cpabe.policyparser.ASTNumericalAttribute;
import cpabe.policyparser.ASTOf;
...
public class PolicyParsing {
	...
	private static void handleNumericalAttribute(String name, boolean greaterThan, BigInteger number, StringBuffer retVal) throws ParseException {
		...
		// bit_marker_list()
		int bits = (number.compareTo(BI_2_32) >= 0 ? 64 : number.compareTo(BI_2_16) >= 0 ? 32 : number.compareTo(BI_2_08) >= 0 ? 16 : number.compareTo(BI_2_04) >= 0 ? 8 : number
				.compareTo(BI_2_02) >= 0 ? 4 : 2);
		int i = 0;
		if (greaterThan)
		{
			while ((1L << i & numberLong) != 0)
				i++;
		}
		else
		{
